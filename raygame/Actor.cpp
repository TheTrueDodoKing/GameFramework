...
bool Actor::removeComponent(const char* componentName)
{
    ...
    for (int i = 0; i < m_componentCount; i++)
    {
        //If this componment is the one we watn to remove...
        if (componentName == m_components[i]->getName())
        {
            ///...mark that we removed tht component and skip
            ///and go to the next iteration fof the loop.
            componentRemoved = true;
            continue;
        }
        //Otherise, copy the component to the new array.
